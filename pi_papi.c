...
int main(int argc, char *argv[]) {
    ...
    if (threads < 1 || n < 1) {
        fprintf(stderr, "Usage: %s [base|cache|icache|vecsp|vecdp] [threads] [N]\n", argv[0]);
        return 1;
    }
    ...
    printf("app,group,threads,actual_threads,size,elapsed_seconds,instructions,cycles,l1_i_misses,l1_d_misses,l2_misses,vec_sp,vec_dp,pi\n");
    if (strcmp(group.name, "base") == 0) {
        printf("pi,%s,%d,%d,%ld,%.9f,%lld,%lld,,,,,,%.12f\n",
               group.name, threads, actual_threads, n, elapsed, totals[0], totals[1], pi);
    } else if (strcmp(group.name, "cache") == 0) {
        printf("pi,%s,%d,%d,%ld,%.9f,,,,%lld,%lld,,,%.12f\n",
               group.name, threads, actual_threads, n, elapsed, totals[0], totals[1], pi);
    } else if (strcmp(group.name, "icache") == 0) {
        printf("pi,%s,%d,%d,%ld,%.9f,,,%lld,,,,,%.12f\n",
               group.name, threads, actual_threads, n, elapsed, totals[0], pi);
